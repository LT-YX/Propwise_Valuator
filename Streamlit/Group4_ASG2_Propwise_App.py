...
# Set Mapbox API key from secrets
...
# MODEL LOADING - From GitHub Repository
@st.cache_resource
...
if artifacts is None:
    st.warning("⚠️ ML model not available. Dashboard features are fully functional.")
    model = None
    label_encoders = None
...
if page == "🧠 Price Prediction":
    ...
    if st.button("🔮 Predict Resale Price", type="primary", use_container_width=True):
        if model is None:
            st.error("⚠️ Price prediction is currently unavailable")
            st.info("📊 Please use our Market Overview dashboards and Amenities Explorer for property insights")
        else:
            input_data = {
